	<div class="setting-header clearfix">
		...
		<div class="lfloat">
			<div class="setting-title" style="line-height: 30px"><?=Translate::Val('Near Transport')?></div>


			<select class="inputtext" id="Types" data-action="filter" name="type" style="display: inline-block;">
					<?php
			...
			foreach ($this->types as $key => $value) {
				...
				$sel = $this->type==$value['id'] ? ' selected':'';
			...
			<tr data-id="<?=$item['id']?>">
				...
				<td class="status">
					...
					foreach ($this->types as $key => $value) {
						...
						$sel = $item['type_id']==$value['id'] ? ' selected':'';
						echo '<option'.$sel.' value="'.$value['id'].'">'. ucfirst($value['name']).'</option>';
					}
