...
foreach ($this->results['lists'] as $key => $value) {
	...
	'<div class="outer">'.
		...
		'<div class="inner">'.
			'<label class="textarea"><textarea class="name" data-plugins="autosize" name="name[]" rows="1">'.trim($value['fullname']).'</textarea></label>'.
			...
			'<label class="textarea"><textarea class="address" data-plugins="autosize" name="address[]" rows="1">'.trim($value['company_address']).'</textarea></label>'.
		'</div>'.
...
#form 
$arr['form']= '<form class="model-formSticker js-print" data-plugins="setsticker" target="_print" action="'.URL.'printer/sticker" method="post"></form>';
